...
def cut(runtime, datap, temp):
    ...
    sum3600num = 0
    sumcutnum = 0
    downlinkL = range(0,11)
    #downlinkL = [0]
    fd = open('/home/wanwenkai/sourceData/SMT-3G/first24h.txt', 'r')
    for line in fd.readlines():
        file_name = 'proUDP_'+line.split()[0]+'.txt'
        print file_name
        for dnum in downlinkL:
            downlink = 'downlink_'+str(dnum*100000)
            print downlink
            data_linkpath = os.path.join(datap, downlink)
            for txt in os.listdir(data_linkpath):
                if txt.endswith('.txt'):
                    if txt == file_name:
                        print txt,
                        filespath = os.path.join(data_linkpath, txt)
                        tL = []
                        bL = []
                        sum_b = 0.0
                        num = 1
...
def divideLevel(doublelineSource):
    ...
    divp = os.path.join(currentPath, 'first24h-30s-doubleline-'+str(levelnum)+'level')
